...
#try:
fromA_B, toA_B, stepA_B, bias_start, bias_end, bias_step, sweep_mode = [float(i) for i in user_params.split(';')]
...
'''if sweep_mode == SWEEP_MODE_INCR:
    fields = np.arange(fromA_B, toA_B, stepA_B)
elif sweep_mode == SWEEP_MODE_DECR:
    fields = np.arange(toA_B, fromA_B, -stepA_B)
elif sweep_mode == SWEEP_MODE_INCR_DECR:
    fields = np.hstack( ( np.arange(fromA_B, toA_B, stepA_B), np.arange(toA_B, fromA_B, -stepA_B) ) )
elif sweep_mode == SWEEP_MODE_DECR_INCR:
    fields = np.hstack( ( np.arange(toA_B, fromA_B, -stepA_B), np.arange(fromA_B, toA_B, stepA_B) ) )
else:
...
tabVB = pw.addScatter2D('V-B (now)', "$B, G$", fr'$U, {core_units[k_V_meas]}V$', markersize=3)
...
def MainThreadProc():
    ...
    for k, v0 in enumerate(v0_sweep):
        curr = (v0 / R) / k_A  # bias current now
        now_current = v0/R

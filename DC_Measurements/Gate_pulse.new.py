...
def MeasureOneVoltage(data):
    tnow = measureTimeNow()
    v = data[:, 6] / gain
    n_data = v.shape[0]

    prev_time = times[-1]
    new_times = np.linspace(prev_time, tnow, n_data)
...
def VoltageMeasurementProc():
    while not f_exit.is_set():
        data = Leonardo.MeasureMany()
        MeasureOneVoltage(data)
        HandleTrigger(data)
...
graph_thread = threading.Thread(target=GraphingThread)
graph_thread.start()

...
Yokogawa_B = YokogawaMeasurer(device_num=19, dev_range='2E-1', what='CURR')
...
try:
    gate_amplitude, gate_points, rangeA_B, stepA_B = [float(i) for i in user_params.split(';')]
...
# -----------------------------------------------------------------
down_line_1B = np.arange(0, -rangeA_B, -stepA_B * 2)
upper_line_1B = np.arange(-rangeA_B, rangeA_B, stepA_B)
down_line_2B = np.arange(rangeA_B, 0.01, -stepA_B * 2)
# -----------------------------------------------------------------
...
def thread_proc():
    ...
    for I_field in upper_line_1B:
        # this field swept value
        FieldUtils.SetField(Yokogawa_B, I_field)
        print('Field is: ', I_to_B(I_field), 'Gs')
        ...
        for i, curr_VG in enumerate(voltValuesGate):
            ...
            def PerformStep(yok, currValues, voltValues,
                ...
                if volt > upper_R_bound:
                    ...
                    last_resistance = UpdateResistance(pw.Axes[tabIV], this_RIValues, this_RUValues)
        ...
        DataSave(I_to_B(I_field))

    # slowly return field again
    ReturnAtExit()
